...
namespace TP5Grupo18
{
    public class Conexion
    {
        ...
        public Sucursal obtenerSucursalPorNombre(string nombre) {
            string consultaSQL = $"SELECT * FROM Sucursal WHERE UPPER(NombreSucursal) = '{nombre.ToUpper()}'";
            DataTable dataTable = this.ejecutarConsulta(consultaSQL);
            Sucursal objSucursal = null;
            if (dataTable.Rows.Count > 0) {
                DataRow row = dataTable.Rows[0];
                objSucursal = new Sucursal(
                    row["Id_Sucursal"].ToString(),
                    row["NombreSucursal"].ToString(),
                    row["DescripcionSucursal"].ToString(),
                    row["DireccionSucursal"].ToString(),
                    row["Id_ProvinciaSucursal"].ToString()
                );
            }
            return objSucursal;
        }
